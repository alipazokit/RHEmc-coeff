...
for (int i=0;i<nrows;i++){
    ...
    //cout<<"ddddddddddddd"<<SEjack(i,0)<<endl;
...
MatrixXdr coeff;
int  read_weight(int Nbin, std::string filename){
        ifstream ifs(filename.c_str(), ios::in);
        std::string line;
        std::istringstream in;
	...
	int Nline=j;
	int Ncol=Nbin;
	coeff.resize(Nbin,Nline);
	j=0;
	ifstream ifs1(filename.c_str(), ios::in);
...
///// compute jackknife for new statisitcs (linear combination of sigma^2s)
string coeff_adr=command_line_opts.Coeff;
int Num_stat=read_weight(Nbin,coeff_adr);
MatrixXdr jack_temp=jack.block(0,0,jack.rows()-1,jack.cols());

MatrixXdr jack_stat;
jack_stat.resize(Num_stat,Njack);
for(int i=0;i<Num_stat;i++){
	jack_stat.row(i)=(jack_temp.array().colwise()*coeff.col(i).array()).colwise().sum();
}
MatrixXdr stat_pointest;
stat_pointest.resize(Num_stat,1);
MatrixXdr sigma_bin=Sigmas.block(0,0,Sigmas.rows()-1,Sigmas.cols());
stat_pointest=((coeff.array().colwise()*sigma_bin.col(0).array()).colwise().sum()).transpose();
MatrixXdr stat_se=jack_se(jack_stat);

cout<<"Coefficients of statistics  :"<<endl;
cout<<coeff.transpose()<<endl;

outfile<<"Coefficients statistics  :"<<endl;
outfile<<coeff.transpose()<<endl<<endl;

for (int i=0;i<Num_stat;i++){

	cout<<i<<"-th statistic:  point estimate: "<<stat_pointest(i,0)<<" SE: "<<stat_se(i,0)<<endl;
	outfile<<i<<"-th statistic:  point estimate: "<<stat_pointest(i,0)<<" SE: "<<stat_se(i,0)<<endl;
}


/*for(int i=0;i<Num_stat;i++){
stat_jack=jack.array().colwise()*Coeff.col(i).array();
MatrixXdr stat_se=jack_se(stat_jack);
cout<<stat_se
}
*/
for (int i=0;i<Njack;i++){
