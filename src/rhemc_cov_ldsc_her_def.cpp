...
cout<<"score-term1: "<<score<<endl;

for (int i=0;i<=Nbin;i++)
	term2+=2*herit(i,0)*Y_r(i,0);

cout<<"score,term2: "<<term2<<endl;
//score=score-(2*herit(i,0)*Y_r(i,0));
for (int i=0;i<=Nbin;i++)
        for(int j=0;j<=Nbin;j++)
                term3+=herit(i,0)*herit(j,0)*X_l(i,j);

cout<<"score,term3: "<<term3<<endl;

score=score-term2+term3;
cout<<"score of mom fitness: "<<score<<endl;    
//////////////////////////////////////////////////
...
for(int i=0;i<Nbin;i++){
    her_per_snp_inbin(i,0)=(double)jack(i,k)/(len[i]-jack_bin[k][i]);          ///error is fixed
}
...
outfile<<"score of mom fitness: "<<score<<endl;
	return 0;
}
